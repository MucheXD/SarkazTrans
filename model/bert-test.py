"""Standard BERT masked-language-model token inspection.

Reads text from stdin, masks each token position in turn, and prints the
top-k token predictions for that position.
"""
...
def get_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Standard BERT token top-k tester")
...
def predict_topk_for_text(
	...
	if len(input_tokens) <= 2:
		raise ValueError("输入太短，无法进行逐 token 预测")

	special_ids = set(getattr(tokenizer, "all_special_ids", []))
	mask_token_id = tokenizer.mask_token_id
	if mask_token_id is None:
		raise RuntimeError("当前 tokenizer 不包含 mask token")

	results: List[Tuple[str, float, List[Tuple[str, float]]]] = []
	for position in range(1, len(input_tokens) - 1):
		original_token_id = input_ids[0, position].item()
		masked_input_ids = input_ids.clone()
		masked_input_ids[0, position] = mask_token_id

		with torch.inference_mode():
			model_kwargs = {"input_ids": masked_input_ids, "attention_mask": attention_mask}
			if token_type_ids is not None:
		...
		results.append((normalize_display_token(input_tokens[position]), source_prob, candidates))
